...
extern "C" void EXTI0_IRQHandler(void)  // Do not forget the ‘extern “C”’ in case of C++
{
    ...
    EXTI->PR = EXTI_PR_PR0;    // Reset interrupt flag of EXTI*. This is needed otherwise the interrupt handler will be called again.
...
int main(void)
{
  ...
  //GPIOA->PUPDR = (GPIOA->PUPDR & ~GPIO_PUPDR_PUPDR1) | (0b10 << GPIO_PUPDR_PUPDR1_Pos);      //set pin PA1 to pull up
    SYSCFG->EXTICR[0] = SYSCFG->EXTICR[0] & ~SYSCFG_EXTICR1_EXTI0; // pin PA0 to interrupt EXTI*
    EXTI->RTSR = EXTI_RTSR_TR0;   // Set interrupt EXTI* trigger to rising edge
